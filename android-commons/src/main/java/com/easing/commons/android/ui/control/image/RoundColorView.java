...
public class RoundColorView extends View {
  ...
  protected void onSizeChanged(int w, int h, int oldw, int oldh) {
    ...
    public void set(Integer circleColor, Integer textColor, String text, Integer dpFontSize) {
        if (circleColor != null)
        ...
        if (text != null)
            this.text = text;
        if (dpFontSize != null)
            textPaint.setTextSize(Dimens.toPx(ctx, dpFontSize));
        invalidate();
    }

    public void set(Integer circleColor, Integer textColor, Integer dpFontSize) {
        if (circleColor != null)
            cirlePaint.setColor(circleColor);
        if (textColor != null)
            textPaint.setColor(textColor);
    ...
    public static int randomColor() {
        int index = Maths.randomInt(0, COLORS.length - 1);
        return COLORS[index];
    }
}
