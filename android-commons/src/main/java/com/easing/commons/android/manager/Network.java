...
public class Network {
    ...
    public static boolean hasNetwork() {
        return hasNetwork(CommonApplication.ctx);
    }
    ...
    public static NetworkInfo getActiveNetworkInfo() {
        ConnectivityManager manager = CommonApplication.ctx.getSystemService(ConnectivityManager.class);
    ...
    public static boolean isMobileNetworkConnected() {
        ...
        return networkInfo != null && networkInfo.getTypeName().equals("MOBILE");
    }

    //通过服务地址连通性，检测服务器连通性
    public static boolean testNetworkByServiceUrl(String url, HttpMethod method) {
        Postman postman = Postman.create().url(url).method(method).connectTimeOut(2000).executeInBlocking();
        if (postman.result.error == null)
            return true;
        if (postman.result.error != CommonEvents.NETWORK_TIMEOUT)
            return true;
        return false;
    }
}
