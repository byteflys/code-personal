...
public class OptionDialogEx1<T> extends DialogFragment {
    ...
    //静态创建方法
    public static <T> OptionDialogEx1<T> create(CommonActivity ctx, T[] options, OnSelectListener<T> listener) {
        OptionDialogEx1 dialog = new OptionDialogEx1();
    ...
    //静态创建方法
    public static <T> OptionDialogEx1<T> create(CommonActivity ctx, T[] options, NameTranslator<T> nameTranslator, OnSelectListener<T> listener) {
        OptionDialogEx1 dialog = OptionDialogEx1.create(ctx, options, listener);
        dialog.nameTranslator = nameTranslator;
        return dialog;
    }

    //显示
    public OptionDialogEx1<T> cancelable(boolean cancelable) {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        for (T option : options) {
            //添加选项文本
            TextView tv = Views.inflate(ctx, R.layout.item_option_dialog_ex1);
